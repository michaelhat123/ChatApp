...
const ProfileEdit = () => {
      ...
      try {
        ...
        img.onerror = () => {
          ...
          const initials = profileData.fullName
            ...
            .join('')
            .toUpperCase() || '??';
          const color = generateColorFromString(profileData.username);
          setProfileData({
            ...profileData,
            email,
  ...
  const handleImageUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      console.log('Uploading file:', {
        ...
        type: file.type,
        size: file.size,
        formData: formData.has('profileImage')
      });
      ...
      if (!response.data || !response.data.profileImage) {
        throw new Error('Invalid response from server: No profile image path received');
      }
  ...
  const handleRemoveProfilePicture = async () => {
    ...
    try {
      ...
      // Generate default avatar with consistent color
      const initials = profileData.fullName
        ?.split(' ')
        .map(name => name?.[0] || '')
        .join('')
        .toUpperCase() || '??';
      const color = generateColorFromString(profileData.username);
      const defaultAvatarUrl = `https://ui-avatars.com/api/?name=${encodeURIComponent(initials)}&background=${color.substring(1)}`;
      
      // Update the profile data with default avatar
      setProfileData(prev => prev ? {
        ...prev,
                      ...
                      <AvatarImage 
                        ...
                        onError={(e) => {
                          ...
                          const initials = profileData?.fullName
                            ...
                            .toUpperCase() || '??';
                          const color = generateColorFromString(profileData?.username || '');
                          e.currentTarget.src = `https://ui-avatars.com/api/?name=${encodeURIComponent(initials)}&background=${color.substring(1)}`;
                        }}
                      />
